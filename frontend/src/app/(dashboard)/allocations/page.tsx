...
export default function AllocationsPage() {
  ...
  const [pendingTransfers, setPendingTransfers] = useState<TransferRequest[]>([
    {
      ...
      tag: "AF-0012",
      from: "J. Mehta (IT)",
      to: "Aditi Rao (Engineering)",
      reason: "Asset needed for frontend framework performance profiling",
      status: "Pending",
    },
  ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newRequest: TransferRequest = {
      id: Date.now(),
      tag: selectedAsset,
      from: selectedAsset === "AF-0119" ? "Priya Shah (Engineering)" : "System Admin",
      to: `${targetEmployee} (${targetEmployee === "Rohan Mehta" ? "Facilities" : targetEmployee === "Sana Iqbal" ? "Field Ops" : "Engineering"})`,
      reason: reason.trim() || "No reason specified",
      status: "Pending",
    };

    setPendingTransfers((prev) => [newRequest, ...prev]);
    setSuccess(true);
    setTargetEmployee("");
    setReason("");
  ...
  return (
    <div className="space-y-6 w-full font-sans">
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {selectedAsset === "AF-0119" && (
            <form onSubmit={handleSubmit} className="bg-white p-6 border-3 border-black rounded shadow-[6px_6px_0_#111110] space-y-4">
              ...
              {success && (
                <div className="bg-green-300 text-black p-3 border-2 border-black rounded font-mono text-xs font-bold text-center animate-fade-in">
                  ✅ Transfer request submitted successfully! (Added to pending list on the right)
                </div>
              )}
            </form>
          )}
        ...
        {/* History and Pending list Column */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 border-3 border-black rounded shadow-[6px_6px_0_#111110] space-y-4">
            <div className="border-b-2 border-black pb-2">
              <h3 className="font-mono text-base font-black uppercase text-yellow bg-black px-2 py-0.5 inline-block">
                Pending Requests ({pendingTransfers.length})
              </h3>
            </div>
            <div className="space-y-4 overflow-y-auto max-h-[360px] pr-1">
              {pendingTransfers.map((req) => (
                <div key={req.id} className="p-3 bg-paper border-2 border-black rounded shadow-[2px_2px_0_#111110] space-y-2">
                  <div className="flex justify-between items-center text-[10px] font-mono font-bold">
                    <span className="bg-black text-[#ffd400] px-2 py-0.5 rounded">{req.tag}</span>
                    <span className="text-blue-600 uppercase bg-blue-50 px-2 py-0.5 border border-blue-600 rounded">
                      {req.status}
                    </span>
                  </div>
                  <div className="text-[11px] font-mono leading-tight">
                    <div className="text-zinc-500">From: <span className="text-black font-bold">{req.from}</span></div>
                    <div className="text-zinc-500">To: <span className="text-black font-bold">{req.to}</span></div>
                  </div>
                  <div className="text-[10px] text-zinc-600 font-mono italic leading-snug border-t border-black/10 pt-1.5">
